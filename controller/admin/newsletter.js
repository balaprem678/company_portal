module.exports = function (app, io) {
    ...
    controller.userList = function (req, res) {
        ...
        if (req.body.search) {
            ...
            userQuery.push({ "$match": { $or: [{ "documentData.phone.number": { $regex: searchs + '.*', $options: 'si' } }, { "documentData.phone.code": { $regex: searchs + '.*', $options: 'si' } }, { "documentData.username": { $regex: searchs + '.*', $options: 'si' } }, { "documentData.email": { $regex: searchs + '.*', $options: 'si' } }] } });
            //search limit
    ...
    controller.driverList = function (req, res) {
        ...
        if (req.body.search) {
            ...
            var searchs = req.body.search;
            //userQuery.push({ "$match": { "documentData.email": { $regex: searchs + '.*', $options: 'si' } } });
            userQuery.push({ "$match": { $or: [{ "documentData.phone.number": { $regex: searchs + '.*', $options: 'si' } }, { "documentData.phone.code": { $regex: searchs + '.*', $options: 'si' } }, { "documentData.username": { $regex: searchs + '.*', $options: 'si' } }, { "documentData.email": { $regex: searchs + '.*', $options: 'si' } }, { "documentData.restaurantname": { $regex: searchs + '.*', $options: 'si' } }] } });
            //search limit
            userQuery.push({ $group: { "_id": null, "countvalue": { "$sum": 1 }, "documentData": { $push: "$documentData" } } });
            userQuery.push({ $unwind: { path: "$documentData", preserveNullAndEmptyArrays: true } });
            if (req.body.limit && req.body.skip >= 0) {
        ...
        db.GetAggregation('drivers', userQuery, function (err, docdata) {
            if (err) {
                res.send(err);
            } else {
                if (docdata.length != 0) {
    ...
    controller.restList = function (req, res) {
        var errors = req.validationErrors();
        if (errors) {
            res.send(errors, 400);
            return;
        ...
        db.GetAggregation('restaurant', userQuery, function (err, docdata) {
            if (err) {
                res.send(err);
            } else {
                if (docdata.length != 0) {
    ...
    controller.emailtemplatelist = async function (req, res) {
        ...
        const docdata= await db.GetAggregation('emailnotifications', newsQuery)
        if (!docdata) {
            res.send(err);
        } else {
    ...
    controller.deletenotification = function (req, res) {
        ...
        db.DeleteDocument('emailnotifications', { _id: { $in: req.body.ids } }, function (err, docdata) {
            if (err) {
                res.send(err);
            } else {
                res.send(docdata);
        ...
        if (type == 'user') {
            ...
            db.GetOneDocument('emailnotifications', { '_id': temid }, {}, {}, function (err, templatedata) {
                if (err) {
                    ...
                    for (var i = 0; i < sendmail.length; i++) {
                        db.GetOneDocument('restaurant', { '_id': sendmail[i] }, {}, {}, function (err, docdata) {
                            if (err) {
                                res.send(err);
                            } else {
                                async.waterfall([
            ...
            db.GetOneDocument('emailnotifications', { '_id': temid }, {}, {}, function (err, templatedata) {
                if (err) {
                    ...
                    for (var i = 0; i < sendmail.length; i++) {
                        db.GetOneDocument('drivers', { '_id': sendmail[i] }, {}, {}, function (err, docdata) {
                            if (err) {
                                res.send(err);
                            } else {
                                async.waterfall([
                    ...
                    for (var i = 0; i < sendmail.length; i++) {
                        db.GetOneDocument('users', { '_id': sendmail[i] }, {}, {}, function (err, docdata) {
                            if (err) {
                                ...
                                if (typeof docdata != 'undefined' && typeof docdata._id != 'undefined') {
                                    var android_provider = docdata._id;
                                    var message = templatedata.name;
                                    var response_time = 250;
                                    var options = { 'subject': templatedata.subject, 'user_id': android_provider, 'message': templatedata.content, 'notification': 'admin_notification' };
                        ...
                        db.GetOneDocument('restaurant', { '_id': sendmail[i] }, {}, {}, function (err, docdata) {
                            if (err) {
                                ...
                                if (typeof docdata != 'undefined' && typeof docdata._id != 'undefined') {
                                    ...
                                    var response_time = 250;
                                    var options = { 'subject': templatedata.subject, 'user_id': android_provider, 'message': templatedata.content, 'notification': 'admin_notification' };
                                    push.sendPushnotification(android_provider, message, 'admin_notification', 'ANDROID', options, 'RESTAURANT', function (err, response, body) { });
                                }
                            }
                        });
                    }
                        ...
                        db.GetOneDocument('drivers', { '_id': sendmail[i] }, {}, {}, function (err, docdata) {
                            if (err) {
                                ...
                                if (typeof docdata != 'undefined' && typeof docdata._id != 'undefined') {
                                    ...
                                    var response_time = 250;
                                    var options = { 'subject': templatedata.subject, 'user_id': android_provider, 'message': templatedata.content, 'notification': 'admin_notification' };
                                    push.sendPushnotification(android_provider, message, 'admin_notification', 'ANDROID', options, 'DRIVER', function (err, response, body) { });
                                }
                            }
                        });
                    }
