module.exports = function () {
    ...
    controller.save = function (req, res) {
        ...
        if (data.avatarBase64) {
            ...
            var file = CONFIG.DIRECTORY_SLIDERS + fileName;
            library.base64Upload({ file: file, base64: base64[2] }, function (err, response) { });
            data.image = attachment.get_attachment(CONFIG.DIRECTORY_SLIDERS, fileName);
            data.img_name = encodeURI(fileName);
            data.img_path = CONFIG.DIRECTORY_SLIDERS.substring(2);
        }

        if (req.body._id) {
            db.UpdateDocument('slider', { '_id': req.body._id }, data, function (err, result) {
                if (err) {
                    res.send(err);
                } else {
                    res.send(result);
