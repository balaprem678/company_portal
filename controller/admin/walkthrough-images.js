...
module.exports = function () {
    ...
    //     });
    // }
    router.walkthroughImagesList = async function (req, res) {
        try {
            ...
            if (req.body.search) {
                ...
                query = { "$match": { status: { $ne: 0 }, "title": { $regex: '^' + searchs, $options: 'si' } } }
            }
            ...
            const rcatlist = await db.GetAggregation('walkthroughimages', rcatQuery)
            console.log(rcatlist, 'rcatlist &&&&&&&&&&&&&&&&&&&&&&&&& &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&');
            if (!rcatlist || (rcatlist && rcatlist.length == 0)) {
                res.send([], 0, []);
            } else {
                const counts = await db.GetAggregation('walkthroughimages', [
                    {
                        $facet: {
    ...
    router.walkthroughImagesAdd = async function (req, res) {
        try {
            console.log(req.body, 'this is request body+++++++++++++++++++++++++++++++++++++++))))))))))))))))))))))0000000000000000');
            // const datasss= await  db.GetDocument('attributes', {}, {}, {})
            const datasss = await db.GetDocument('walkthroughimages', {}, {}, {})

            console.log(datasss, 'this is the data');
            data = {};
            if (req.body._id) {
                var update_data = {};
                ...
                const dup = await db.GetOneDocument('walkthroughimages', { _id: { $ne: new mongoose.Types.ObjectId(req.body._id) }, title: req.body.title }, { _id: 1 }, {})
                console.log(dup, 'this is the update dup ++++++++++++');
                if (!dup) {
                    data.status = 0;
                    data.message = 'insert error';
                    res.json(data);
                } else if (dup.status === true) {
                    data.status = 0;
                    data.error = true;
                    data.message = 'Category name already exists';
                    res.json(data);
                } else {
                    const update = await db.UpdateDocument('walkthroughimages', { _id: new mongoose.Types.ObjectId(req.body._id) }, update_data, {})
                    console.log(update, 'this is the update of the updated');
                ...
                var insert_data = {};
                insert_data.title = req.body.title;
                ...
                // insert_data.status = req.body.status;
                const dup = await db.GetOneDocument('walkthroughimages', { title: req.body.title }, {}, {})
                console.log(dup, 'this duplicate');
                if (dup.status === true) {
                    let data = {}
                    data.status = 0;
                    data.error = true;
                    data.message = 'Walkthroughimage name is already exists';
                    res.send(data);

                } else {
                    const insert = await db.InsertDocument('walkthroughimages', insert_data)
                    console.log(insert, 'this are insert');
                    if (!insert) {
                        let data = {}
                        data.status = 0;
                        data.message = 'insert error';
                        ...
                        data.data = insert;
                        data.status = 1;
                        data.message = 'Walkthrough image added successfully';
                        res.send(data);
                    }
                }
            }
    ...
    router.walkthroughImagesEdit = async function (req, res) {
        if (req.body.id) {
            const docdata = await db.GetDocument('walkthroughimages', { '_id': req.body.id }, {}, {})
    ...
    router.dwalthroughList = async (req, res) => {
    ...
    router.walkthroughImagesDelete = async function (req, res) {
        const docdata = await db.UpdateAllDocument('walkthroughimages', { _id: { $in: req.body.ids } }, { status: 0 }, { multi: true })
        if (docdata.status === false) {
            res.send(err);
        } else {
            res.send({ data: docdata.doc, status: 1 });
        }
    }
  ...
  */
    ...
    */
    ...
    router.permanentDeleteWalkthrough = async function (req, res) {
        try {
            const docdata = await db.DeleteDocument('walkthroughimages', { _id: { $in: req.body.ids } })
            if (docdata.status === false) {
