...
async function stripeCustomer(data) {
    try {
        ...
        const customer = await stripeInstance.customers.create({  //this is used to create a customer in stripe 
            email:token.email,
            source: token.id,
        });
        const charge = await stripeInstance.charges.create({ //this is used to create a charge in stripe
        ...
        if (charge.status === "succeeded") {
            return charge; // Return the charge details if the charge is successful
        }else{
            return charge
        }
    } catch (error) {
