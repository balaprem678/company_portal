...
@Component({
  selector: 'app-combo-add-edit',
  templateUrl: './combo-add-edit.component.html',
  styleUrls: ['./combo-add-edit.component.scss']
})
export class ComboAddEditComponent {
  colorOptions = [
  ...
  pageTitle: string = 'Add Combo Offer';
  ...
  onFormSubmit(tagAddEditForm: UntypedFormGroup) {
    ...
    if (!this.viewpage) {
      if (tagAddEditForm.valid) {
        ...
        formData.append('comboName', this.userDetails.first_name);
        ...
        formData.append('comboPrice', this.userDetails.comboPrice);
        ...
        this.apiService
          .CommonApi(Apiconfig.comboSave.method, Apiconfig.comboSave.url, datas)
          .subscribe((result) => {
            if (result.status !== 0) {
