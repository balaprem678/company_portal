...
export class OrderpickupComponent implements OnInit {
  ...
  constructor(private router: Router, private authService: AuthenticationService,
    ...
    if (this.curentUser && this.curentUser.doc.role == "subadmin") {
      if (this.router.url == '/app/orders/orderpickup' && this.curentUser.doc.privileges) {
  ...
  changeStatus(id, status) {
    if (id && status) {
      this.apiService.CommonApi(Apiconfig.updateOrderStatus.method, Apiconfig.updateOrderStatus.url, { order_id: id, status: 7 }).subscribe(result => {
        if (result && result.status == 1) {
