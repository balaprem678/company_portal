...
export class RefundedComponent {
      ...
      columns: {
        ...
        phone: {
          ...
          valuePrepareFunction: (value, row) => {
            ...
            return row && row.user && row.user.phone && (row.user.phone.code != (null || undefined || ''))? row.user.phone.code + ' ' + row && row.user && row.user.phone && (row.user.phone.number != (null || undefined || '')) && row.user.phone.number : "-";
          }
        },
        // driver: {
        //   title: 'Driver Name',
        ...
        status: {
          ...
          onComponentInitFunction: (instance: any) => {
            instance.save.subscribe(row => {
              this.changeStatus(row, row.status);
            });
          }
        },
        createdAt: {
  ...
  changeStatus(id, status) {
    console.log(id, 'this is the row');
