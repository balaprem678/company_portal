...
export class CollectedComponent {
  ...
  onexportChange(event) {
    ...
    data.file_name='collected_orders';
        ...
        status: {
          ...
          onComponentInitFunction: (instance: any) => {
            instance.save.subscribe(row => {
              this.changeStatus(row._id,row.foods.id, row.foods.status, row);
            });
          }
        },
        createdAt: {
  ...
  changeStatus(id,product_id, status, row) {
    if (id && status) {
      this.apiService.CommonApi(Apiconfig.returnOrderStatus.method, Apiconfig.returnOrderStatus.url, { order_id: id, status: 18, row:row }).subscribe(result => {
