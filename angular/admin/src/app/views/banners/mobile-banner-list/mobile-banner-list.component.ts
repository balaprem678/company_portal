...
@Component({
  selector: 'app-mobile-banner-list',
  templateUrl: './mobile-banner-list.component.html',
  styleUrls: ['./mobile-banner-list.component.scss']
})
export class MobileBannerListComponent implements OnInit {
  status: any
  getnewuserslist: any[];
  totalItems: any;
  ...
  constructor(private router: Router,
    ...
    if (this.curentUser && this.curentUser.role == "subadmin") {
      if (this.router.url == '/app/banners/mobile-list' && this.curentUser.privileges) {
  ...
  getUsers(getusers) {
    this.apiService.CommonApi(Apiconfig.mobilelistBanners.method, Apiconfig.mobilelistBanners.url, getusers).subscribe(response => {
      if (response && response.length > 0 && response[0] != 0) {
        ...
        this.loadsettings('')
        this.apiService.CommonApi(Apiconfig.banner_mob_archieve.method, Apiconfig.banner_mob_archieve.url, getusers).subscribe(result => {
