...
@Component({
  ...
  styleUrls: ['./interestlist.component.scss']
})
export class InterestlistComponent implements OnInit {
  ...
  delete_btn = true;
  addBtnUrl: string = '/app/interests/add';
  addBtnName: string = 'Interest Add';
  editUrl: string = '/app/interests/edit/';
  deleteApis: apis = Apiconfig.interestDelete;
  card_details: any[] = [];
  ...
  constructor(
    ...
    if (this.curentUser && this.curentUser.role == "subadmin") {
      if (this.router.url == '/app/interests/list') {
  ...
  loadsettings(event) {
    if (event == 'delete') {
      ...
      this.settings.actions.custom = this.getSettings.loadSettings(event, this.curentUser, '/app/interests/list', this.userPrivilegeDetails, this.delete_btn, this.edit_btn, false);
    } else {
      this.settings = {
        // selectMode: 'multi',
        hideSubHeader: true,
