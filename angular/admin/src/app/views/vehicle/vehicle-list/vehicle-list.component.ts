...
export class VehicleListComponent implements OnInit {
  ...
  addBtnUrl: string = '/app/vehicle/add';
  addBtnName: string = 'Add';
  editUrl: string = '/app/vehicle/edit/';
  viewUrl: string = '/app/vehicle/view/';
  global_status: number = 0;
  global_search: string;
  deleteApis: apis = Apiconfig.deletevehicle;
  constructor(
    ...
    if (this.curentUser && this.curentUser.role == "subadmin" && this.curentUser.privileges) {
      if (this.router.url == '/app/vehicle/list') {
  ...
  getDataList(data) {
    this.apiService.CommonApi(Apiconfig.getvehicle.method, Apiconfig.getvehicle.url, data).subscribe(
      (result) => {
  ...
  loadsettings(event) {
    ...
    this.settings.actions.custom = this.getSettings.loadSettings(event, this.curentUser, '/app/vehicle/list', this.userPrivilegeDetails, this.delete_btn, this.edit_btn, this.view_btn);
  }
